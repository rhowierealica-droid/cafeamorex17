...
let currentDriverStoredPassword = null;
...
async function loadDriverProfile(email) {
    ...
    try {
        ...
        if (snapshot.empty) {
            alert("Driver account not found or access denied.");
        ...
        currentDriverUID = docSnap.id;
        currentDriverEmail = data.email;
        currentDriverStoredPassword = data.password; 
        const fullName = data.firstName && data.lastName
            ? `${data.firstName} ${data.lastName}`
            : data.fullname || "Driver User";
...
document.addEventListener('DOMContentLoaded', () => {
    const storedEmail = sessionStorage.getItem("driverEmail");
    loadDriverProfile(storedEmail);
});
...
passwordForm?.addEventListener("submit", async (e) => {
    e.preventDefault();
    if (!currentDriverUID || !currentDriverEmail) return alert("Session Error. Please refresh.");
    ...
    if (currentPassword !== currentDriverStoredPassword) {
    ...
    try {
        ...
        currentDriverStoredPassword = newPassword; 
