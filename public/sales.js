...
onAuthStateChanged(auth, async (user) => {
  ...
  await loadOrdersRealtime(); // load realtime data
});
...
// ==========================
// Load orders in REALTIME from Firestore
// ==========================
...
async function loadOrdersRealtime() {
  ...
  for (const col of collections) {
    ...
    const unsubscribe = onSnapshot(colRef, (snapshot) => {
      // Remove old data for this collection to avoid duplicates
      ...
      console.log(`Realtime update from ${col}: ${snapshot.docs.length} docs`);
      renderDashboard();
    });
