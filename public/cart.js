...
        // NOTE: This function relies on item having correct currentQty, sizeQty etc.
        // It's usually safer to fetch the current inventory inside this function 
        // before committing the batch update to prevent race conditions.
        // However, keeping the original logic structure for now.
        
        for (const ing of item.ingredients || []) {
            if (ing.id) {
...
            customerEmail: currentUser.email || "", // ⭐ Added email
...
            // ⭐ FIX: Use a single orderData object to match the Netlify function's logic
            const orderData = {
                items: commonOrderData.items,
                cartItemIds: commonOrderData.cartItemIds,
                total: commonOrderData.total, // Total (including delivery) for comparison
                deliveryFee: commonOrderData.deliveryFee,
                userId: commonOrderData.userId,
                customerName: commonOrderData.customerName,
                customerEmail: commonOrderData.customerEmail, // Pass email to the function
...
                body: JSON.stringify({
                    orderData, // ⭐ Pass the orderData object
                    amount: Math.round(orderTotal * 100), // Total amount in cents/centavos
