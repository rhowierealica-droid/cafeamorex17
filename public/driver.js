...
          password = sessionStorage.getItem("driverPassword");
        }
...
        if (snapshot.empty) {
          alert("Access Denied. Invalid or unauthorized Driver account.");
...
      if (selectedStatus === "Completed" && order.completedAt) {
        const date = order.completedAt.toDate();
        const formatted = date.toLocaleString("en-US", {
          year: "numeric", month: "short", day: "numeric",
          hour: "2-digit", minute: "2-digit"
        });
        completedDateHtml = `<p>Completed: <strong>${formatted}</strong></p>`;
      }
...
          <p><strong>Phone:</strong> <a href="tel:${order.phoneNumber || order.customerPhone || ""}">
            ${order.phoneNumber || order.customerPhone || "N/A"}</a></p>
...
                <i class="fas fa-check-circle"></i> Mark Completed</button>` : ""}
...
                <i class="fas fa-receipt"></i> View/Download Receipt</button>` : ""}
        </div>
      `;
