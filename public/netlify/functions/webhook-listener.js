...
            // Search by payment ID AND check for status "Refund Pending"
            const querySnapshot = await db.collection(col)
...
        if (finalStatusUpdate.status) { // Check if a status update was defined
            await orderRef.update(finalStatusUpdate);
        }

        return { statusCode: 200, body: JSON.stringify({ received: true, processedRefund: true, orderId: orderRef.id }) };
    }
