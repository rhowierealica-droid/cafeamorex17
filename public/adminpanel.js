    ...
    const processAllData = () => {
        ...
        allOrdersData.forEach(data => {
            ...
            if (data.status && data.status.toLowerCase().includes("completed")) {
                ...
                let orderProductSales = data.total || 0;
                let deliveryFee = 0;

                if (data.collection === "DeliveryOrders" && data.deliveryFee) {
                    deliveryFee = data.deliveryFee;
                    orderProductSales = (data.total || 0) - deliveryFee;
                }

                if (orderProductSales < 0) orderProductSales = 0;
                

                if (createdAt >= start && createdAt <= end) {
                    totalOrdersToday++;
                    totalSalesToday += orderProductSales;
                    ...
                    hourlyData[hour][collectionType] += orderProductSales;
                }
...
function getStockStatus(item) {
    ...
    if (["Ingredients", "Adds-on", "Others"].includes(item.category)) {
        ...
        if (["slice", "squeeze", "Piece"].includes(item.unit))
            return qty === 0 ? "Out of Stock" : qty < 10 ? "Low Stock" : "In Stock";
        if (item.unit === "scoop")
            return qty === 0 ? "Out of Stock" : qty < 5 ? "Low Stock" : "In Stock";
    }

    if (item.category === "Sizes" && item.unit === "pieces")
        return qty === 0 ? "Out of Stock" : qty < 30 ? "Low Stock" : "In Stock";
