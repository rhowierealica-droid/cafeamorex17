    ...
    function renderCartItems() {
      ...
      cartItems.forEach(item => {
        ...
        checkbox.addEventListener("change", e => {
          ...
          selectAllCheckbox.checked = selectedCartItems.size === cartItems.length;
          updateCartTotal();
          updateModalTotals();
        });
...
async function loadSavedAddresses() {
  ...
  try {
    ...
    if (userDoc) {
      ...
      if (defaultAddr) {
        cartAddresses.push({ fullAddress: defaultAddr, deliveryFee: Number(userDoc.deliveryFee ?? 0) });
...
finalConfirmBtn?.addEventListener("click", async () => {
  ...
  try {
    ...
    if (paymentMethod === "GCash") {
      const res = await fetch("https://cafeamorex17.netlify.app/.netlify/functions/create-payment-link", {
        method: "POST",
